...
const CSS = [
  ...
  ".navlinks a{text-decoration:none;color:#4b5563;font-size:.9rem;font-weight:500;cursor:pointer}",
  ".navlinks a:hover{color:#0f4c5c}",
  ".navcta{background:#f97316;color:#fff;border:none;padding:.6rem 1.4rem;border-radius:.4rem;font-weight:700;cursor:pointer;font-size:.9rem}",
  ...
  ".trust span::before{content:\"\u2713 \";color:#16a34a;font-weight:700}",
  ...
  ".field label{display:block;font-weight:600;color:#374151;margin-bottom:.45rem;font-size:.92rem}",
  ...
  ".subbtn{width:100%;background:#f97316;color:#fff;border:none;padding:1rem;border-radius:.5rem;font-size:1.05rem;font-weight:700;cursor:pointer;margin-top:.4rem}",
  ".subbtn:hover{background:#ea6c00}",
...
export default function App() {
  ...
  const check = () => {
    ...
    const diff = r - MARKET;
    setResult({ ...v, r, diff: diff.toFixed(2), lo: Math.max(0,Math.round(diff*0.8*parseFloat(form.amount)/100)), hi: Math.max(0,Math.round(diff*parseFloat(form.amount)/100)) });
    go("result");
  };
  const submit = async () => {
    if (!lead.name || !lead.phone || !lead.email) return;
    try { await fetch("https://hook.eu1.make.com/your-webhook-url", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({source:"ratemyhomeloan",...lead,...form,verdict:result?.label}) }); } catch(e){}
    setDone(true);
  };
  ...
  return (
    <>
      ...
      <nav className="nav">
        ...
        <button className="navcta" onClick={()=>go("quiz")}>Check My Rate</button>
      </nav>
      {page==="home" && <>
        <div className="hero">
      ...
      {page==="quiz" && <div className="sec">
        ...
        <div className="card">
          ...
          <div className="field"><label>Your Current Interest Rate (%)</label>
            <input type="number" step="0.01" min="1" max="15" placeholder="e.g. 6.45" value={form.rate} onChange={e=>setForm({...form,rate:e.target.value})} /></div>
          <div className="field"><label>Loan Amount ($)</label>
            <input type="number" step="10000" min="50000" placeholder="e.g. 600000" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})} /></div>
          ...
          <button className="subbtn" onClick={check}>Deliver the Verdict &#8594;</button>
        </div>
      </div>}
      {page==="result" && result && <div className="sec">
        ...
        <div className="card" style={{textAlign:"center"}}>
          ...
          {!done ? <div className="lf">
            ...
            <button className="subbtn" onClick={submit}>Get My Free Rate Review &#8594;</button>
